...
@foreach ($carouselImageNames as $name)
  <div>
    <a href="/images/photos/jackets/linden/carousel/@{{ getColorName(leather_color) }}/{{{ $name }}}_large.jpg" title="">
      <img src="/images/photos/jackets/linden/carousel/@{{ getColorName(leather_color) }}/{{{ $name }}}_medium.jpg" alt="{{{ $name }}}">
    </a>
  </div>
@endforeach
...
@section('main')
  <div class="row">
    @foreach (['fur', 'interior-pocket', 'waist'] as $name)
      <img class="large-6 medium-6 small-12 columns" src="/images/photos/jackets/linden/details/@{{ getColorName(leather_color) }}/{{{ $name }}}.jpg" alt="{{{ $name }}}">
    @endforeach
