  ...
  @foreach ($carouselImageNames as $name)
    <div>
      <a href="/images/photos/jackets/e-161/carousel/{{{ $name }}}_large.jpg" title="">
        <img src="/images/photos/jackets/e-161/carousel/{{{ $name }}}_medium.jpg" alt="{{{ $name }}}">
      </a>
    </div>
  @endforeach
@stop
...
@section('main')
  <div class="row">
    @foreach (['collar', 'interior-pocket', 'waist'] as $name)
      <img class="large-6 medium-6 small-12 columns" src="/images/photos/jackets/e-161/details/{{{ $name }}}.jpg" alt="{{{ $name }}}">
    @endforeach
